  ...
  \subcaptionbox{\label{fig2.3a}}[0.48\textwidth]
    {
      \begin{tikzpicture}[thick,>=Stealth]
        \draw [->] (-0.5,0) -- (0,0)node[below left]{$O$} -- (4,0);
        \draw [->] (0,-0.5) -- (0,3.8);
        \draw (0.6,0.9)node[below left,inner sep=1pt]{$z_0$} -- node[below]{$|\dz|$}(2.4,0.9) -- (2.4,2.3) -- cycle;
  ...
  \subcaptionbox{\label{fig2.3b}}[0.48\textwidth]
    {
      \begin{tikzpicture}[thick,>=Stealth]
        ...
        \begin{scope}[scale = 1.1]
          \draw (0.6,0.9 )node[below left,inner sep=1pt]{$w_0$} -- node[below]{$|\dif f|$} (2.4,0.9) -- (2.4,2.3) -- cycle;
          \draw (0.9,0.9) arc (0:40:0.3);
  ...
  \subcaptionbox{\label{fig2.4a}}[0.48\textwidth]
    {
      \begin{tikzpicture}[thick, > = Stealth]
        ...
        \draw [->] (0,0) -- (0,4);
        \draw (-2,3) -- (0,3) node[above right]{$2\pi\ii$} -- (2,3);
        \draw (-2,1) -- (0,1)node[above right]{$y_0\ii$} -- (2,1);
      \end{tikzpicture}
    }
...
\[
  w_k'(z) = \ee^{\mu\varphi_k(z)} \mu\varphi_k'(z) = \mu z^{\mu-1} = \mu\ee^{(\mu-1)\varphi_k(z)}.
\]

有了上面这些知识，我们就可以作一些简单的保角变换了.

\begin{example}\label{exam2.4.5}
  求一保角变换，把除去线段$\{z=a+\ii y:0<y<h\}$的上半平面变为上半平面.
\end{example}
\begin{solution}
  \begin{figure}[!ht]
    \centering
    \begin{tikzpicture}[thick,> = Stealth]
      ...
      \begin{scope}[xshift = 5cm]
        \draw (-2,0) -- (0,0) node[below]{$O$} -- (2,0) (0,0) -- (0,1) node[above]{$h\ii $};
        \fill (0,1) circle (1pt);
      \end{scope}
  ...
  \begin{equation*}
    w = \sqrt{z_3} = \sqrt{z_2+h^2} = \sqrt{z_1^2+h^2} = \sqrt{(z-a)^2+h^2}. \qedhere
  \end{equation*}
\end{solution}
...
\begin{figure}[!ht]
  \centering
    \begin{tikzpicture}[thick,> = Stealth, scale = 0.8]
      ...
      \draw(-3,2)--(0,2)node[above]{$2\ii $}--(3,2);
      \draw(-3,1)--(-1,1)node[above]{$-1+\ii $};
      ...
      \begin{scope}[xshift=10cm,yscale=pi/2]
        \draw(-3,0)--(0,0)node[below]{$O$}--(3,0);
        \draw(-3,2)--(0,2)node[above]{$\pi\ii $}--(3,2);
        \draw(-3,1)--(-1,1)node[above]{$-\frac\pi2+\frac\pi2\ii $};
      \end{scope}
      \fill[xshift=10cm](0,0)circle(1pt)(0,pi)circle(1pt)(-1,pi/2)circle(1pt);
      \begin{scope}[yshift=-3cm]
        \draw(-3,0)--(0,0)node[below]{$O$}--(3,0)(0,0)
             --(0,1)node[above]{$\ee ^{-\ii \frac\pi2}$};
        \fill(0,0)circle(1pt)(0,1)circle(1pt);
        \draw[->](-4,1)--node[above]{$z_2=\ee ^{z_1}$}(-2,1);
      \end{scope}
      \draw[->,yshift=-3cm](3.5,pi/2)--node[above]{$w=\sqrt{z_2^2+\ee ^{-\pi}}$}(6.5,pi/2);
  ...
  \subcaptionbox{\label{fig2.12a}}[0.48\textwidth]
    {
      \begin{tikzpicture}[thick,>=Stealth,every node/.style={inner sep=1.5pt}]
        \draw(0,0)node[below]{$O$}circle(1)(1,0)node[right]{$1$}circle(2)
            (-1,0)node[left]{$-1$}(3,0)node[right]{$3$}(0,1)node[above]{$\ii $};
     ...
     \begin{minipage}[b]{0.48\linewidth}
       ...
       \begin{tikzpicture}[thick,>=Stealth,every node/.style={inner sep=1.5pt},scale=2]
         ...
         \draw(-1,0)node[left]{$-1$}(0,0)node[right]{$O$}
              (1,0)node[right]{$1$}(0,0)--(0,-2)node[right]{$-2\ii $};
