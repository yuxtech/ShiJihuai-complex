...
\draw(0,0)--(2,0)node[above right,inner sep=1pt]{$\textrm e^{\textrm i\theta_0}$};
...
\draw[->](1,-0.4)node[below]{$S_\alpha(\textrm e^{\textrm i\theta_0})$}--++(0,0.7);
\end{tikzpicture}
...
\draw(0,0)node[below left]{$O$}circle(2);
\draw[->](0,0)--(2,0)--(3.2,0);
\draw[->](0,0)--(80:2)node[above]{$\;\textrm e^{\textrm i\theta}$}
...
\end{align*}
当$2\varphi=\pi-\theta,\varphi=\frac{\pi-\theta}2$. 这样，由 \eqref{eq4.2.5} 式便可得
\begin{align*}
...
\end{equation}
一般来说，当$\Gamma$是一条不通过原点的任意可求长闭曲线时，$\frac1{2\pi\ii}\int\limits_\Gamma \frac{\textrm dw}w$等于$\Gamma$绕原点的圈数，称为$\Gamma$关于原点的\textbf{环绕指数}\index{H!环绕指数}，因而 \eqref{eq4.4.4} 式对于一般的不通过原点的可求长闭曲线都是成立的.
...
>={Stealth[width=3pt]}]
\draw(4,0)node[below]{$R$}--(0,0)node[below left]{$O$}--(0,4)node[left]{$R\textrm i$};
\draw[->](4,0)--(5,0);
\draw[->](0,4)--(0,5);
...
最大模原理的一个重要应用是可以用它来证明下面\textbf{Schwarz引理}.\index{D!定理!Schwarz引理}
