...
设$D$是域，$\gamma_1,\cdots,\gamma_n$是$\MC$中的$n$条可求长曲线.如果$f$在$D$上连续，在开集$D\backslash\big(\operatorname*{\cup}_{k=1}^n\gamma_k\big)$上全纯，那么$f$必在$D$上全纯.
\end{theorem}
\begin{proof}
...
Schwarz对称原理可以推广到更一般的情形.为了叙述方便，对$\MC_\infty$中的圆周$\gamma$分别用$\MC_\infty^+(\gamma)$和$\MC_\infty^-(\gamma)$表示$\MC_\infty$被$\gamma$分成的两个单连通域.实际上，$C_\infty^+(\gamma)$和$\MC_\infty^-(\gamma)$就是圆周$\gamma$的两侧.
...
\big(\operatorname*{\cup}_{k=-\infty}^\infty L_k\big)$是如图 \ref{fig6.6} 所示的单连通域，将$D$双全纯地映为上半平面.
\begin{figure}[!ht]
\centering
